...
# --- 2. REPRODUCE THE STRATEGY LOGIC ---
PULLBACK = 40
BOUNCE = 45
SL_ATR = 2.5
TP_ATR = 0.8
        ...
        if not in_trade:
            ...
            if trade_dir == 1:
                ...
                if exit_px is not None:
                    trades.append({'pnl': exit_px - entry_price, 'bars_held': bars_held, 'entry': entry_price, 'exit': exit_px, 'dir': trade_dir, 'idx': i})
                    in_trade = False
            else:
                if time_arr[i].hour >= 15 and time_arr[i].minute >= 10: exit_px = curr_close
                ...
                if exit_px is not None:
                    trades.append({'pnl': entry_price - exit_px, 'bars_held': bars_held, 'entry': entry_price, 'exit': exit_px, 'dir': trade_dir, 'idx': i})
                    in_trade = False
    return trades
...
# (A) No same-bar entry/exit
same_bar = t_df['bars_held'].min() == 0
# (B) No NaN or inf in PnL
nan_pnl = t_df['pnl'].isna().any() or np.isinf(t_df['pnl']).any()
# (C) No duplicate trades at same index
dupes = t_df['idx'].duplicated().any()
# (D) No trades with entry==exit
zero_pnl = (t_df['entry'] == t_df['exit']).any()

print("--- STRATEGY LOGIC VALIDATION ---")
print(f"Same-bar entry/exit: {'BUG' if same_bar else 'OK'}")
print(f"NaN or inf in PnL: {'BUG' if nan_pnl else 'OK'}")
print(f"Duplicate trade indices: {'BUG' if dupes else 'OK'}")
print(f"Zero PnL trades: {'BUG' if zero_pnl else 'OK'}")
