# 📊 PROMETHEUS — Monthly Performance Tracker

**Tracking Period:** August 2026  
**Initial Base Capital:** Rs 100,000 (Option C 10-Day Crucible)  
**Current Realized Net P&L:** **🔴 -Rs 26,708.87**  
**Last Updated:** 2026-09-23 17:00:00 IST  

---

## 1. Month-to-Date Performance Overview

| Metric | Value | Benchmark / Target | Status |
| :--- | :---: | :---: | :---: |
| **Net Realized P&L** | **-Rs 26,708.87** | +Rs 5,000 / month | 🔴 |
| **Total Trading Days** | **19** | 20–22 days | In Progress |
| **Total Trades** | **64** | ~3–5 / day | Tracked |
| **Gross Profit** | +Rs 26,572.00 | — | Winning Trades |
| **Gross Loss** | -Rs 51,717.46 | — | Losing Trades |

---

## 2. Daily Performance Log

| Date | Trades | W / L | Win Rate | Gross Profit | Gross Loss | Net P&L | Daily Return | Cumulative P&L | Key Notes & Market Context |
| :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **2026-08-25** | 4 | 1W / 3L | 25% | +Rs 335 | -Rs 1,250 | **🔴 -Rs 915.0** | -6.1% | **-Rs 915.0** | NIFTY 24150PE (+28% trailed to +Rs335), NIFTY R2 SL (-Rs465), MIDCAP SL (-Rs651), SENSEX test (-Rs134) |
| **2026-08-26** | 12 | 8W / 4L | 67% | +Rs 11,199 | -Rs 6,507 | **🟢 +Rs 4,691.9** | +31.3% | **+Rs 3,776.9** | 8 Credit Spread Wins (+Rs 11199), 4 Momentum Buying Losses (-Rs 6507) |
| **2026-08-27** | 6 | 0W / 6L | 0% | +Rs 0 | -Rs 4,453 | **🔴 -Rs 4,458.1** | -29.7% | **-Rs 681.2** | Midday mean-reversion squeeze; 5 Nifty PE buys and 1 Sensex PE buy hit SL |
| **2026-08-28** | 6 | 2W / 4L | 33% | +Rs 4,411 | -Rs 2,561 | **🟢 +Rs 1,850.4** | +12.3% | **+Rs 1,169.2** | 2 Credit Spread Wins (+Rs 4411), 4 Afternoon PE momentum buys hit SL (-Rs 2561) |
| **2026-08-31** | 4 | 0W / 4L | 0% | +Rs 0 | -Rs 4,454 | **🔴 -Rs 4,459.9** | -29.7% | **-Rs 3,290.7** | Morning PE breakout reversed; 2 monthly Sensex PE and 2 Nifty PE hit SL |
| **2026-09-01** | 6 | 5W / 1L | 83% | +Rs 7,128 | -Rs 251 | **🟢 +Rs 6,877.2** | +45.8% | **+Rs 3,586.6** | 5 Credit Spread Wins (+Rs 7128), 1 Expiry FastTrigger PE SL (-Rs 251) |
| **2026-09-02** | 3 | 0W / 3L | 0% | +Rs 0 | -Rs 6,410 | **🔴 -Rs 6,412.9** | -42.8% | **-Rs 2,826.3** | Afternoon upward trend breached Bear Call short strikes; defined-risk SL capped loss |
| **2026-09-03** | 6 | 0W / 6L | 0% | +Rs 0 | -Rs 9,850 | **🔴 -Rs 9,854.5** | -65.7% | **-Rs 12,680.8** | Paper ledger recorded fallback; live exchange closing quotes showed +Rs 3378 profit |
| **2026-09-04** | 6 | 2W / 4L | 33% | +Rs 1,379 | -Rs 7,023 | **🔴 -Rs 6,181.5** | -41.2% | **-Rs 18,862.3** | 2 Credit Spreads gained (+Rs 1128 net decay); 4 Call buys lost on afternoon pullback (-Rs 7310) |
| **2026-09-07** | 0 | 1W / 0L | 0% | +Rs 0 | -Rs 0 | **⚪ +Rs 0.0** | +0.0% | **-Rs 18,862.3** | Intraday paper trading session |
| **2026-09-08** | 0 | 2W / 0L | 0% | +Rs 0 | -Rs 0 | **⚪ +Rs 0.0** | +0.0% | **-Rs 18,862.3** | Intraday paper trading session |
| **2026-09-09** | 1 | 1W / 0L | 100% | +Rs 203 | -Rs 0 | **🟢 +Rs 75.8** | +0.5% | **-Rs 18,786.5** | NIFTY 15 SEP Bear Call Spread 23600CE/23750CE: Entry net credit 49.32 exit 46.20 (+Rs 75.82 net; live Kite was flat -Rs 107 gross / ~-Rs 207 net due to midday rally offsetting theta). Golden setup option buying appropriately suppressed in 50pt morning chop. |
| **2026-09-10** | 2 | 2W / 0L | 100% | +Rs 1,489 | -Rs 0 | **🟢 +Rs 1,239.3** | +8.3% | **-Rs 17,547.2** | SENSEX 75000/75300 CE spread (+Rs 660.15 net, target decay hit @ 15.05); MIDCPNIFTY 14625/14700 CE spread (+Rs 579.18 net, square-off @ 27.40) |
| **2026-09-11** | 1 | 0W / 1L | 0% | +Rs 0 | -Rs 6,730 | **🔴 -Rs 6,881.1** | -45.9% | **-Rs 24,428.3** | Deep gap-down (-0.8%) & massive V-reversal (+1351pt Bank Nifty). Morning MIDCAP PE trade caught on stale SQLite candle; system hardened to strictly reject stale cache during market hours. |
| **2026-09-15** | 0 | 0W / 0L | 0% | +Rs 0 | -Rs 0 | **⚪ +Rs 0.0** | +0.0% | **-Rs 24,428.3** | Intraday paper trading session |
| **2026-09-16** | 0 | 1W / 0L | 0% | +Rs 0 | -Rs 0 | **⚪ +Rs 0.0** | +0.0% | **-Rs 24,428.3** | Intraday paper trading session |
| **2026-09-21** | 3 | 1W / 2L | 33% | +Rs 211 | -Rs 1,418 | **🔴 -Rs 1,347.1** | -1.3% | **-Rs 25,775.3** | Day 1: 3 Bear Call Spreads (Tier C); 1 BE win (+Rs 87.47 net), 2 SL (-Rs 1434.53). 100% paper. |
| **2026-09-22** | 0 | 0W / 0L | 0% | +Rs 0 | -Rs 0 | **⚪ +Rs 0.0** | +0.0% | **-Rs 25,775.3** | Day 2: 0 trades executed. Angle One missing 23800CE hedge contract identified and resolved. |
| **2026-09-23** | 4 | 1W / 3L | 25% | +Rs 217 | -Rs 811 | **🔴 -Rs 933.5** | -0.9% | **-Rs 26,708.9** | Day 3: BN 56500CE SL (-Rs902.84); BN 56600CE trailed win (+Rs8.64); NIFTY BE (-Rs26.37); SENSEX BE (-Rs12.97). Trailing stop saved ~Rs4500 from afternoon 600pt flush. |

---

## 3. Data File Locations

* **CSV Raw Ledger:** [`reports/papertrade/daily_performance_ledger.csv`](file:///c:/Users/amanu/Desktop/Trading/reports/papertrade/daily_performance_ledger.csv)
* **Markdown Tracker:** [`reports/papertrade/monthly_performance_tracker.md`](file:///c:/Users/amanu/Desktop/Trading/reports/papertrade/monthly_performance_tracker.md)