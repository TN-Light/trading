...
def main():
    ...
    rows = []
    mode_trades = {"swing_15m": [], "intraday": []}

    for sym in symbols:
        data_primary = prom.data.fetch_historical(sym, days=30, interval="15minute", force_refresh=False)
        data_hourly = prom.data.fetch_historical(sym, days=30, interval="60minute", force_refresh=False)
        ...
        if data_primary is None or data_primary.empty:
            rows.append({"mode": "swing_15m", "symbol": sym, "trades": 0, "win_rate": 0.0,
                         "net_pnl": 0.0, "avg_profit": 0.0, "avg_loss": 0.0, "note": "no data"})
        else:
            engine_s15 = run_swing_15m(prom, sym, data_primary, data_hourly, data_daily)
            trades = filter_trades(getattr(engine_s15, "trades", []), start, end)
            mode_trades["swing_15m"].extend(trades)
            stats = summarize(trades)
            rows.append({"mode": "swing_15m", "symbol": sym, **stats})
