...
DEFAULT_PERIODS = [
    ...
    {"label": "covid_2020", "start": datetime(2020, 2, 1), "end": datetime(2020, 5, 31)},
...
def resolve_period_window(data, period):
    ...
    if period.get("label") == "full":
        ...
        end_dt = period.get("end") or max_dt
...
def run_scenario_job(job):
    symbol, scenario_label, slip, csv_dir, config_path = job
    ...
    for period in DEFAULT_PERIODS:
        ...
        stats = summarize_trades(trades, start_dt, end_dt)
        rows.append({
            "scenario": scenario_label,
            "period": period["label"],
            "symbol": symbol,
...
def run_symbol_job(job):
    symbol, csv_dir, config_path, stress_slippage = job
...
def parse_args():
    parser = argparse.ArgumentParser(description="Reality check: swing logic on 15-minute data.")
    parser.add_argument("--symbols", nargs="*", default=DEFAULT_SYMBOLS)
    parser.add_argument("--csv-dir", default="dataset")
    parser.add_argument("--processes", type=int, default=0)
    parser.add_argument("--parallel-scenarios", action="store_true")
    parser.add_argument("--stress-slippage", type=float, default=0.30)
...
def main():
    ...
    if args.parallel_scenarios:
        jobs = [
            (sym, label, slip, csv_dir, config_path)
            for sym in symbols
            for label, slip in scenarios
        ]
        worker = run_scenario_job
    else:
        jobs = [(sym, csv_dir, config_path, args.stress_slippage) for sym in symbols]
