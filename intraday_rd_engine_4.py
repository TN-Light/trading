...
def generate_alpha_features(df):
    ...
    # Daily Compression Filters (Day T-1)
    daily = df.groupby('date').agg({'high':'max', 'low':'min'})
    daily['range'] = daily['high'] - daily['low']
    daily['nr4'] = (daily['range'] < daily['range'].shift(1)) & (daily['range'] < daily['range'].shift(2)) & (daily['range'] < daily['range'].shift(3))
    daily['prev_nr4'] = daily['nr4'].shift(1)
    daily['avg_range_10'] = daily['range'].rolling(10).mean().shift(1)
    daily['is_squeeze'] = daily['range'].shift(1) < (0.75 * daily['avg_range_10'])
    
    df = df.reset_index().merge(daily[['prev_nr4', 'is_squeeze']], on='date', how='left').set_index('datetime')
...
def test_open_to_close_strategy(df, tax_drag=15.0):
    ...
    for i in range(14, len(df)-1):
        if not in_trade:
            ...
            # REGIME FILTER: Was yesterday compressed?
            if not (prev_nr4_arr[i] or is_squeeze_arr[i]):
            ...
            if trend_up and close_arr[i] > ib_h_arr[i]:
                ...
                entry_price = open_arr[i+1] # Open of 10:30 candle
...
print('=== 15-MIN TIMEFRAME R&D (PHASE 5: FILTERED DAY-T1 COMPRESSION) ===')
for sym, tax, lot in [('NIFTY 50', 15.0, 25), ('NIFTY BANK', 35.0, 15), ('NIFTY FIN SERVICE', 15.0, 40)]: 
    ...
    if df is not None:
        ...
        evaluate(df_tc, tax, f'{sym[:9]} Squeeze Trend', lot)