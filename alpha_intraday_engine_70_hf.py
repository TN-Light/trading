...
print("Loading dataset for High Frequency 70%+ Winrate Engine...")
...
def run_backtest(df, rsi_pullback_thresh, rsi_bounce_thresh, sl_atr, rr_ratio):
    ...
    for i in range(14, len(df)-1):
        if not in_trade:
            # Shifted to 09:30 to catch early momentum structure
            if time_arr[i].hour >= 15 or time_arr[i].hour < 9 or (time_arr[i].hour == 9 and time_arr[i].minute < 30):
                continue
            
            # Less strict alignment: Just EMA > EMA and VWAP, ADX > 15
...
print("Hunting High-Freq 70%+ Win Rate (500 days) ...")
rsi_pullbacks = [40, 45, 50, 55]
rsi_bounces = [45, 50, 55, 60]
sl_atrs = [1.5, 2.0, 2.5]
rr_ratios = [0.3, 0.4, 0.5, 0.6]
...
for p, b, sl, rr in product(rsi_pullbacks, rsi_bounces, sl_atrs, rr_ratios):
    ...
    # Needs a minimum of 300 to ~400+ trades
    # (350 trades across ~350 trading days = ~1 trade per day = ~20 trades/month)
    if cnt >= 280:
        results.append({
...
if len(df_res) > 0:
    ...
    print("\n Top High Frequency 70%+ Winrate Configurations ")
